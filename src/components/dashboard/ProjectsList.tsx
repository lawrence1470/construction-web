  ...
  return (
    <div className="bg-white rounded-3xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-medium text-gray-800">Active Projects</h2>
          ...
          <div key={project.id} className="border-b border-gray-100 pb-4 last:border-0">
            <div className="flex items-start justify-between mb-2">
              <div className="flex-1">
                <p className={`text-gray-800 mb-1 ${project.completed ? 'line-through' : ''}`}>
                  {project.name}
                </p>
                <p className="text-sm text-gray-500">{project.description}</p>
              </div>
              {project.completed && (
                <div className="w-8 h-8 bg-[#c8d97e] rounded-full flex items-center justify-center">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex -space-x-2">
                {project.assignees.map((avatar, idx) => (
                  <div key={idx} className="w-7 h-7 rounded-full overflow-hidden border-2 border-white">
                    <ImageWithFallback
                      src={avatar}
                      alt="Team member"
